...
Jupyter queda disponible en `http://localhost:8888` (revisa el log del contenedor para el token: `docker compose logs jupyter`). El Spark Master UI queda en `http://localhost:8080`.

> **Importante:** la imagen `jupyter/pyspark-notebook` no incluye `prophet`, `duckdb` ni `scikit-learn` por defecto. Después de levantar el contenedor, hay que instalar las dependencias extra una vez (ver sección de Requisitos previos más abajo) dentro del contenedor de Jupyter:
> ```bash
...
Dentro del contenedor de Jupyter, instalar las dependencias que no vienen en la imagen base:

```bash
docker compose exec jupyter pip install -r requirements.txt --break-system-packages
```

(`requirements.txt` está en la raíz del repo — incluye `prophet`, `duckdb`, `scikit-learn` y `pandas` con las versiones exactas usadas en el desarrollo.)
